...
##############################################################
    ...
    def setStopFunction(self, function: object) -> object:
        ...
        if box.kind == "ticks":
            ...
            if count > 0:
                ...
                if index < 0 or index > count - 1:
                    self.warn("Invalid option: %s. Should be between 0 and %s." , index, count-1)
                else:
...
#                file_name = image.split('.jpg', 1)[0] + '.ppm'
                param = 6
            else:
#                file_name = image.split('.jpg', 1)[0] + '.pgm'
#                file_name = "test3.pgm"
                param = 5
...
#        self.config(padx=1, pady=1, bd=0)
        ...
        if self.resize:
            ...
            self.canvas.bind('<Configure>', self._updateFrameWidth) # ScrollPane width bug fix
            self.interior.bind('<Configure>', self._updateWidth)    # Thanks to Tom "tgolsson" Solberg

    def _updateFrameWidth(self, event): # ScrollPane width bug fix
        self.canvas.itemconfig(self.interior_id, width=event.width) # ScrollPane width bug fix
